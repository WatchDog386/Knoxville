 ...
 * @property {string} siteTitle - Website title (e.g., "knoxvilletechnologies")
...
const settingSchema = new mongoose.Schema({
  siteTitle: {
    type: String,
    default: 'knoxvilletechnologies', // ✅ Updated default to match your brand
    trim: true,
    maxlength: [100, 'Site title cannot exceed 100 characters']
  },
  adminEmail: {
    type: String,
    default: 'info@knoxvilletechnologies.com', // ✅ Updated to match your website's contact email
    trim: true,
    lowercase: true,
    match: [
      /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/,
...
settingSchema.statics.getSettings = async function() {
  ...
  if (!settings) {
    settings = await this.create({
      siteTitle: 'knoxvilletechnologies',
      adminEmail: 'info@knoxvilletechnologies.com',
      language: 'en'
    });
